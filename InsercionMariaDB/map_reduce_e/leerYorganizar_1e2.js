...
connection.connect((err) => {
  ...
  fs.readFile('part-00000-2e', 'utf-8', (err, data) => {
    if (err) {
    ...
    datosOrganizados.forEach(([pagina, cantidad]) => {
      obtenerIdPagina(pagina, (error, idPagina) => {
        ...
        const insertImagenQuery = 'INSERT INTO imagen (id_pagina, alt) VALUES (?, ?)';
        const values = [idPagina, 'prueba'];
    
        connection.query(insertImagenQuery, values, (error, results, fields) => {
          if (error) {
            console.error('Error al insertar datos en la tabla "imagen":', error);
          } else {
            console.log('Datos insertados con éxito en la tabla "imagen".');
          }
